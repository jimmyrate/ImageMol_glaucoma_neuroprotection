...
class ImageDataset_tsne(Dataset):
    def __init__(self, filenames, index=None, img_transformer=None, normalize=None, ret_index=False, args=None):
        '''
        :param filenames: image path, e.g. ["./data/1.png", "./data/2.png", ..., "./data/n.png"]
        :param img_transformer: transformations to apply to images
        :param normalize: normalization to apply to images
...
def load_filenames_multitask(image_folder, txt_file):
    df = pd.read_csv(txt_file)
    index = df["index"].values.astype(int)
    names = [os.path.join(image_folder, str(item) + ".png") for item in index]
    return names
