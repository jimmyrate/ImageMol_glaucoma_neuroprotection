...
def main():
    ...
    parser.add_argument('--dataroot', type=str, default="/root/autodl-tmp/ImageMol/datasets/glaucoma_candidate/", help='data root')
    parser.add_argument('--dataset', type=str, default="raw", help='dataset name, e.g. data')
    parser.add_argument('--dataset_name', type=str, default="glaucoma_candidates", help='dataset name, e.g. data')

    args = parser.parse_args()

    raw_file_path = os.path.join(args.dataroot, args.dataset, "{}.csv".format(args.dataset_name))
    img_save_root = os.path.join(args.dataroot, args.dataset, "224")
    csv_save_path = os.path.join(args.dataroot, args.dataset, "{}_for_train.csv".format(args.dataset_name))
    ...
    index, smiles, label = df['index'].values, df['SMILES'].values, df['Label'].values
    ...
    for i, s, l in tqdm(zip(index, smiles, label), total=len(index)):
        ...
        try:
            ...
            processed_ac_data.append([i, filename, l])
        except:
            pass
